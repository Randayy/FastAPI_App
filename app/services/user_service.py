...
class UserService:
    ...
    async def get_user_by_id(self, user_id: int) -> UserDetailSchema:
    ...
    async def update_user(self, user_id: int, user_data: UserUpdateRequestSchema) -> UserDetailSchema:
        user = await self.User_Repository.get_user_by_id(user_id)
        ...
        if user_data_entered['new_password']:
            hashed_password = await self.hash_password(user_data_entered['new_password'])
            user_data_entered['password'] = hashed_password
