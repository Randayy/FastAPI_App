...
@user_router.get("/get-my-quiz-results/{quiz_id}")
...
@user_router.get("/get-my-list-of-avarage-marks")
...
@user_router.get("/get-my-submited-quizzes")
...
@user_router.get("/get-my-notifications")
async def get_my_notifications(db: AsyncSession = Depends(get_session), current_user: User = Depends(get_current_user_from_token)):
    service = UserService(db)
    current_user_id = current_user.id
    notifications = await service.get_my_notifications(current_user_id)
    return notifications

@user_router.post("/mark-notification-as-read/{notification_id}")
async def mark_notification_as_read(notification_id: UUID, db: AsyncSession = Depends(get_session), current_user: User = Depends(get_current_user_from_token)):
    service = UserService(db)
    current_user_id = current_user.id
